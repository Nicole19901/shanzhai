...
<h2>启动流程向导</h2>
<div class="steps">
<div class="step"><div class="step-num" id="sn1">1</div><div class="step-body"><div class="step-title">校验 API Key</div><div class="step-desc">在「API 密钥管理」中填入 Key / Secret，点击「验证余额和持仓」确认可以连上 Binance 账户</div></div></div>
<div class="step"><div class="step-num" id="sn2">2</div><div class="step-body"><div class="step-title">绑定密钥</div><div class="step-desc">验证通过后点「验证并应用」，将 Key 写入运行中的交易客户端；或保存后用「激活」切换</div></div></div>
<div class="step"><div class="step-num" id="sn3">3</div><div class="step-body"><div class="step-title">确认交易对</div><div class="step-desc">在「交易对切换」中输入目标币种并验证，确认 Binance 合约存在后再切换（有持仓时拒绝切换）</div></div></div>
<div class="step"><div class="step-num" id="sn4">4</div><div class="step-body"><div class="step-title">调节参数后启动</div><div class="step-desc">调整止盈止损、杠杆、引擎阈值等参数，点「保存参数」，然后点「启动系统」</div></div></div>
</div>
</div>
...
<div class="trade-notice">
  手动操作<strong style="color:#c9d1d9">绕过引擎信号</strong>直接下单，但仍受以下安全条件限制：<br>
  · 必须已绑定有效 API Key&nbsp;&nbsp;· 当前无持仓（做多/做空）或当前有持仓（平仓）&nbsp;&nbsp;· 系统状态允许开仓
</div>
<div style="display:flex;gap:16px;flex-wrap:wrap;align-items:flex-start">
  <div class="trade-btns" style="flex:3;min-width:300px">
    <div class="trade-btn-wrap">
      <div class="trade-btn-label"><strong>做多 / 开多</strong>以市价买入，押注价格上涨。会自动挂止损单和止盈单保护持仓。</div>
      <button class="btn-long" id="manualLongBtn">买入做多 (Long)</button>
    </div>
    <div class="trade-btn-wrap" style="margin-top:12px">
      <div class="trade-btn-label"><strong>做空 / 开空</strong>以市价卖出，押注价格下跌。会自动挂止损单和止盈单保护持仓。</div>
      <button class="btn-short" id="manualShortBtn">卖出做空 (Short)</button>
    </div>
    <div class="trade-btn-wrap" style="margin-top:12px">
      <div class="trade-btn-label"><strong>平仓 / 强制平仓</strong>以市价关闭当前全部持仓，同时撤销所有止损止盈挂单。无持仓时会报错。</div>
      <button class="btn-close-pos" id="manualCloseBtn">市价平仓 (Close)</button>
    </div>
  </div>
  <div style="flex:1;min-width:180px;background:#0d1116;border:1px solid #29333d;border-radius:6px;padding:12px;font-size:13px">
    <div style="color:#a9b7c4;margin-bottom:8px;font-weight:600">当前持仓状态</div>
    <div id="tradePosDetail" style="color:#8b949e">加载中...</div>
  </div>
</div>
...
document.getElementById('resetBtn').onclick=async()=>{if(!confirm('恢复到初始化参数？'))return;const r=await fetch('/api/params/reset',{method:'POST'});if(r.ok){const d=await r.json();populate(d.current);msg('已恢复初始化值',true)}else msg('恢复失败',false)};document.getElementById('initBtn').onclick=async()=>{if(!confirm('把当前参数保存为新的初始化值？'))return;const r=await fetch('/api/params/initialize',{method:'POST'});msg(r.ok?'当前参数已保存为初始化值':'初始化失败',r.ok)};document.getElementById('startBtn').onclick=async()=>{if(!confirm('确认启动系统服务？'))return;const r=await fetch('/api/control/start',{method:'POST'});msg(r.ok?'已发送启动命令':'启动命令失败: '+await r.text(),r.ok)};document.getElementById('restartBtn').onclick=async()=>{if(!confirm('确认重启系统服务？'))return;const r=await fetch('/api/control/restart',{method:'POST'});msg(r.ok?'已发送重启命令':'重启命令失败: '+await r.text(),r.ok)};document.getElementById('stopBtn').onclick=async()=>{if(!confirm('确认停止系统服务？'))return;const r=await fetch('/api/control/stop',{method:'POST'});msg(r.ok?'已发送停止命令':'停止命令失败: '+await r.text(),r.ok)};
const tradeMsg=document.getElementById('tradeMsg');
document.getElementById('manualLongBtn').onclick=async()=>{if(!confirm('确认以市价买入做多？\n\n系统将按当前参数（保证金/杠杆）下单，并自动挂止损和止盈保护单。'))return;const r=await fetch('/api/trade/open',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({direction:'long'})});msg(r.ok?'做多指令已发送，请在交易日志中确认成交':'做多失败: '+await r.text(),r.ok,tradeMsg)};
document.getElementById('manualShortBtn').onclick=async()=>{if(!confirm('确认以市价卖出做空？\n\n系统将按当前参数（保证金/杠杆）下单，并自动挂止损和止盈保护单。'))return;const r=await fetch('/api/trade/open',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({direction:'short'})});msg(r.ok?'做空指令已发送，请在交易日志中确认成交':'做空失败: '+await r.text(),r.ok,tradeMsg)};
document.getElementById('manualCloseBtn').onclick=async()=>{if(!confirm('确认强制平仓当前全部持仓？\n\n将以市价关闭持仓并撤销所有止损/止盈挂单。此操作不可撤销。'))return;const r=await fetch('/api/trade/close',{method:'POST'});msg(r.ok?'平仓指令已发送，请在交易日志中确认成交':'平仓失败: '+await r.text(),r.ok,tradeMsg)};
...
function renderTradePosDetail(positions){const el=document.getElementById('tradePosDetail');if(!positions||!positions.length){el.innerHTML='<span style="color:#56d364">无持仓 — 可做多或做空</span>';return}const actives=positions.filter(p=>Math.abs(parseFloat(p.amount||0))>0.000001);if(!actives.length){el.innerHTML='<span style="color:#56d364">无持仓 — 可做多或做空</span>';return}let html='';actives.forEach(p=>{const pnl=parseFloat(p.pnl||0);const cls=p.side==='LONG'?'#56d364':p.side==='SHORT'?'#ff7b72':'#8b949e';html+='<div style="margin-bottom:6px"><span class="pos-badge '+(p.side==='LONG'?'pos-long':p.side==='SHORT'?'pos-short':'pos-flat')+'">'+p.side+'</span> <span style="color:#d6dee6">'+p.symbol+'</span><br><span style="color:#8b949e;font-size:12px">数量 '+p.amount+' · 入场 '+parseFloat(p.entry).toFixed(2)+'</span><br><span style="color:'+(pnl>=0?'#56d364':'#ff7b72')+';font-size:12px">未实现 PnL: '+(pnl>=0?'+':'')+pnl.toFixed(4)+' USDT</span></div>';});el.innerHTML=html}
(async()=>{try{const r=await fetch('/api/params');const d=await r.json();populate(d.current)}catch(e){msg('加载参数失败: '+e,false)}loadLogs();loadKeys();loadSymbol();setInterval(loadLogs,3000);setInterval(loadSymbol,10000)})();
</script>
</body>
</html>`
