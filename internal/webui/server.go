...
<section class="card"><h2>交易对切换</h2><div style="display:flex;gap:6px;align-items:flex-end;flex-wrap:wrap"><div class="field" style="flex:1;min-width:140px;margin:0"><label for="symbolInput">输入币种，如 ETH 或 ETHUSDT</label><input id="symbolInput" type="text" placeholder="ETHUSDT" autocomplete="off" spellcheck="false"></div><button class="btn-verify" type="button" id="validateSymbolBtn">验证</button><button class="btn-apply" type="button" id="applySymbolBtn" disabled>切换</button></div><div id="symbolResult" class="muted" style="margin-top:6px"></div><div class="muted" style="margin-top:4px">有持仓时拒绝切换；切换后数据流、OI、下单均跟随新交易对。</div>
