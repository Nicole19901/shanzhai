...
<section class="card"><h2>交易对切换</h2><div style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap"><div class="field" style="flex:1;min-width:180px;margin:0"><label for="symbolInput">输入币种，如 BTC 或 BTCUSDT</label><input id="symbolInput" type="text" placeholder="BTCUSDT" autocomplete="off" spellcheck="false"></div><button class="btn-verify" type="button" id="validateSymbolBtn">验证币种</button><button class="btn-apply" type="button" id="applySymbolBtn" disabled>切换</button></div><div class="field" style="max-width:280px;margin-top:8px"><label for="symbolConfirm">防误触确认：输入验证后的完整交易对</label><input id="symbolConfirm" type="text" placeholder="例如 BTCUSDT" autocomplete="off" spellcheck="false"></div><div id="symbolResult" class="muted" style="margin-top:8px"></div><div class="muted" style="margin-top:8px">验证通过后可热切换行情、OI、下单与对账交易对；已有持仓时会拒绝切换。</div></section>
